...
def _estimate_source_cr_marginalised(log_src_crs_grid, src_counts, bkg_counts, bkg_area, \
    ...
    if weights.sum() <= 0:
        print("WARNING: Weight problem! sum is" , weights.sum(), \
            np.log10(src_counts.max() / rate_conversion), log_src_crs_grid[0], log_src_crs_grid[-1])
    weights /= weights.sum()
...
def bexvar_from_table(LightCurve):
    ...
    Parameters
    ----------
    LightCurve : :class:`astropy.table.Table` object
        Contains light curve for which Bayesian excess variance is to be calculated.
        It must have nonzero columns with names "Counts
    ...
    # Initializing an empty array which will contain the posterior samples of
    # Beysian excess varience of count rates of each band.
    posterior_log_sigma_src_cr = []
    ...
    for band in range(nbands):
        ...
        posterior_log_sigma_src_cr.append(logcr_sigma)
