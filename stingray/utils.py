...
def _addSpectra(final_spectra, curr_spec, flag):
    ...
    if isinstance(final_spectra, AveragedPowerspectrum):
        final_spectra.nphots += curr_spec.nphots

    elif isinstance(final_spectra, AveragedCrossspectrum):
...
def createChunkedSpectra(spec_type, segment_size, norm, gti, power_type, silent, dt):
    """
    ...
    if spec_type == 'AveragedPowerspectrum':
        fin_spec = AveragedPowerspectrum()

        if segment_size < times.chunks[0] / 8192:
            warnings.warn(f"It is advisable to have the segment size greater than or equal to {times.chunks[0] / 8192:}. Very small segment sizes may greatly increase computation times.")

        for i in range(times.chunks[0], times.size, times.chunks[0]):
            lc = Lightcurve(time=times.get_basic_selection(slice(i - times.chunks[0], i)), counts=counts.get_basic_selection(slice(i - times.chunks[0], i)), err=count_err.get_basic_selection(slice(i - times.chunks[0], i)), err_dist=str(err_dist[...]), mjdref=mjdref[...], dt=dt[...], skip_checks=True)

            avg_pspec = AveragedPowerspectrum(data=lc, segment_size=times.chunks[0] / segment_size, norm=norm, gti=gti, silent=silent, dt=dt, large_data=False)
        ...
        if segment_size < times.chunks[0] / 4096:
            warnings.warn(f"It is advisable to have the segment size greater than or equal to {times.chunks[0] / 4096:}. Very small segment sizes may greatly increase computation times.")

        fin_spec = AveragedCrossspectrum()
        for i in range(times.chunks[0], times.size, times.chunks[0]):
            lc1 = Lightcurve(time=times.get_basic_selection(slice(i - times.chunks[0], i)), counts=counts.get_basic_selection(slice(i - times.chunks[0], i)), err=count_err.get_basic_selection(slice(i - times.chunks[0], i)), err_dist=str(err_dist[...]), mjdref=mjdref[...], dt=dt[...], skip_checks=True)
            ...
            avg_cspec = AveragedCrossspectrum(data1=lc1, data2=lc2, segment_size=times.chunks[0] / segment_size, norm=norm, gti=gti, power_type=power_type, silent=silent, dt=dt, large_data=False)
